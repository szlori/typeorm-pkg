        ...
        Object.keys(orderBys).forEach((orderCriteria) => {
            if (orderCriteria.indexOf(".") !== -1) {
                ...
                orderByObject[this.escape(parentAlias) +
                    "." +
                    this.escape(DriverUtils_1.DriverUtils.buildAlias(this.connection.driver, aliasName, column.databaseName)) +
                    rawFieldSpecs] = orderBys[orderCriteria];
            }
            else {
                if (this.expressionMap.selects.find((select) => select.selection === orderCriteria ||
                    select.aliasName === orderCriteria)) {
            ...
            if (embed) {
                ...
                if (relationValue === true ||
                    ...
                    if (relation.inverseEntityMetadata.eagerRelations.length > 0) {
                        relationValue = FindOptionsUtils_1.FindOptionsUtils.mergeFindOptionsRelationsWithEagerRelations(relationValue === true ? {} : relationValue, relation.inverseEntityMetadata.eagerRelations);
                    }
